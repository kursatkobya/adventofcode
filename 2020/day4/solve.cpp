...
public:
    ...
    bool isPassPortReallyValid(Passport pass)
    {
        try{
            std::vector<std::string> requiredFields { "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"};
...
{
    int count{0};
    for (auto item: m_passportList) {
        if (isPassPortValid(item)) ++count;
    }
    return count;
}
...
{ 
    int count{0};
    for (auto item: m_passportList) {
        if (isPassPortReallyValid(item)) ++count;
    }
    return count;
}
