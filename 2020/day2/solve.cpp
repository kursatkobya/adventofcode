...
int Solution::solveA()
{
    ...
    while (testVec.size())
    {
        ...
        for (int i = 0; i < item.password.size(); ++i) {
            if (item.password[i] == item.key) ++count;
        }
        if (item.min <= count && count <= item.max) ++validPwd;
        testVec.pop_back();
    }    
...
{ 
    ...
    while (m_inputVector.size())
    {
        ...
        if (item.password[item.min-1] == item.key) min_exist = true;        
        if (item.password[item.max-1] == item.key) max_exist = true;
        if (min_exist xor max_exist) ++validPwd;
